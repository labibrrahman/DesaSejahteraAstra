...
const JuriLayout = () => {
  ...
  const sidebarContent = (
    <>
      <div style={{ height: 72, padding: collapsed ? '16px 13px' : '16px 20px', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 38, height: 38, background: '#1e293b', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ position: isMobile ? 'relative' : 'absolute', top: isMobile ? 0 : 72, bottom: isMobile ? 0 : 104, left: 0, right: 0, overflowY: 'auto', overflowX: 'hidden', padding: '8px' }}>
        <ConfigProvider theme={{ components: { Menu: { itemSelectedBg: '#2563eb', itemSelectedColor: '#ffffff', itemHoverBg: '#eff6ff', itemBorderRadius: 8 } } }}>
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      {isMobile ? (
        ...
        <Sider trigger={null} collapsible collapsed={collapsed} width={220} collapsedWidth={64} style={{ height: '100vh', position: 'fixed', left: 0, top: 0, bottom: 0, background: '#fff', borderRight: '1px solid #f0f0f0', overflow: 'hidden' }}>
          {sidebarContent}
        </Sider>
      )}
